from tkinter import *
###################IMPORTING THE PASSDATA FILE FOR LOGIN FUNCTIONS ######################################
...
def log():
    ...
    if(p.read(user,passw)):
        if (user == p.username and passw == p.passcode):
    ...
    ############## IF VERIFICATION FAILS IT AGAIN ASKS FOR PASSWORD   #################################
        ...
        l.grid(column=2, row=3, columnspan=2)
...
ps_ent.grid(column=1,row=1,ipadx=5,ipady=5,padx=8,columnspan=4,sticky=W)
...
but_login.grid(column=2,row=2,sticky=(E),pady=20)
